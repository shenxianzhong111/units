...
class Database {
    ...
    public function backup($table, $start) {
        ...
        if ($count) {
            ...
            $columns_null = array_column($columns, 'Null', 'Field');

            foreach ($result as $row) {

                // 解决字段为null的时候导出的sql为‘’无法再次导入的问题
                $data = [];
                array_walk_recursive($row, function($v, $k) use(&$data, $columns_null) {

                    if ($columns_null[$k] == 'YES' && $v === NULL) {
                        $data[$k] = 'NULL';
                    } else {
                        $data[$k] = $v;
                    }
