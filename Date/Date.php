...
class Date {
    ...
    public static function format($format = null, $dateTime = null) {
        return date($format ? $format : 'Y-m-d H:i:s', self::get_timestamp($dateTime));
    }
     ...
     */
    public static function date_part($interval, $dateTime = null) {
        return date($interval, self::get_timestamp($dateTime));
    }
     ...
     * @return string 
     */
    public static function date_diff($interval, $startDateTime, $endDateTime) {
        $diff = self::get_timestamp($endDateTime) - self::get_timestamp($startDateTime);
        $retval = 0;
        switch ($interval) {
            case "y":
     ...
     */
    public static function date_add($interval, $value, $dateTime, $format = null) {
        $date = getdate(self::get_timestamp($dateTime));
        switch ($interval) {
            case "y":
                $date["year"] += $value;
                break;
     ...
     */
    public static function get_monthday($year) {
        $months = array(31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31);
        if (self::get_leapyear($year)) {
            $months[1] = 29;
        }
        return $months;
    }
     ...
     * @return int
     */
    public static function get_monthinday($month, $year) {
        if (1 > $month || 12 < $month) {
            return 0;
        }
        if (!($daysInmonths = self::get_monthday($year))) {
            return 0;
        }
     ...
     * @return int 
     */
    public static function get_daysinyear($year) {
        return self::get_leapyear($year) ? 366 : 365;
    }
     ...
     * @return string
     */
    public static function get_rfc_date($date = null) {
     ...
     * @return string
     */
    public static function get_chinadate($time = null) {
        list($y, $m, $d, $w, $h, $_h, $i) = explode(' ', date('Y n j w G g i', $time ? $time : time()));
        return sprintf('%s年%s月%s日(%s) %s%s:%s', $y, $m, $d, self::get_chinaweek($w), self::get_periodtime($h), $_h, $i);
    }
     ...
     * @return string
     */
    public static function get_chinaweek($week = null) {
     ...
     * @return string
     */
    public static function get_periodtime($hour = null) {
     ...
     * @return string
     */
    public static function get_utc_dtime($dateTime = null) {
        $oldTimezone = self::getTimezone();
        if ('UTC' !== strtoupper($oldTimezone)) {
            self::set_timezone('UTC');
        }
        $date = date('D, d M y H:i:s e', self::get_timestamp($dateTime));
        if ('UTC' !== strtoupper($oldTimezone)) {
            self::set_timezone($oldTimezone);
        }
        return $date;
    }
     ...
     * @return int
     */
    public static function get_microtime($mircrotime = null, $get_as_float = false) {
     ...
     */
    public static function get_leapyear($year) {
     ...
     * @return int
     */
    public static function get_timestamp($dateTime = null) {
     ...
     * @return array
     */
    public static function get_lastdate($time, $timestamp = null, $format = null, $type = 1) {
